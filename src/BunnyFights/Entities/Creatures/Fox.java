...
public class Fox extends Creature {
    ...
    public void rotateFox(Player player) {

        if (((player.getX() < (this.getX()+50)) && (player.getX() > (this.getX()-50))) && player.getY() > this.getY()) {
            image = Assets.foxFace;
        }
        else if (player.getX() > (this.getX()+50)) {
            image = Assets.foxRight;
        }
        else {
            image = Assets.foxLeft;
        }

        if (((player.getX() < (this.getX()+50)) && (player.getX() > (this.getX()-50))) && player.getY() < this.getY()) {
            image = Assets.foxBack;
        }
    }

    public void foxPlayerRelation(Player player) {
        tick();
        moveToPlayer(player);
        rotateFox(player);
    }
